<?php
    ...
    $uid = $_SESSION['uid'];
    $result = $con->prepare('SELECT id, fullname, phone_number, operator, kode, harga, transaction_date FROM transaction WHERE WEEKOFYEAR(transaction_date) = WEEKOFYEAR(NOW()) AND uid = "'.$uid.'"');
    $result->execute();
    $data = array();
