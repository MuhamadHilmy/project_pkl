<?php
    ...
    $uid = $_SESSION['uid'];
    $result = $con->prepare('SELECT id, fullname, phone_number, operator, kode, harga, transaction_date FROM transaction WHERE YEAR(transaction_date) = YEAR(NOW()) AND MONTH(transaction_date) = MONTH(NOW()) AND DAY(transaction_date) = DAY(NOW()) AND uid = "'.$uid.'"');
    $result->execute();
    $data = array();
